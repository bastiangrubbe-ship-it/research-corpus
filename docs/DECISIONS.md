...
New Agents Needed" did not appear in the top 15 for the query `multiplayer`, although
the word is in its title and in six of its own chunks. Cause: `document_summary.text` is
extractive TextRank over the transcript, and on unrehearsed long-form speech TextRank
extracts filler — that summary opens "Uh, and we're just going to dive into the ways I
think you could use it". Zero of his 71 summaries contain the word "multiplayer". Both
summary-reading lanes therefore could not see a term the document plainly carries.

**Tried:** prefix `Document.title` to the summary, so `text` and `embedding` are derived
from one string carrying both. Re-derived all 4,121 summaries (41 minutes, local).

**Measured** — `scripts/paired_retrieval_ab.py`, frozen judgments, 230 relevant over 12
queries, top_k=20:

| lane | P before | P after | ΔP | R before | R after | ΔR |
|---|---:|---:|---:|---:|---:|---:|
| lexical | 0.497 | 0.503 | +0.005 | 0.181 | 0.183 | +0.002 |
| dense | 0.317 | 0.250 | **-0.067** | 0.385 | 0.341 | **-0.044** |
| chunk_dense (control) | 0.371 | 0.371 | +0.000 | 0.432 | 0.432 | +0.000 |
| reranked | 0.379 | 0.354 | **-0.025** | 0.483 | 0.439 | **-0.043** |

**Reverted.** The control lane held at 0.000 in both directions, so the harness is
sound and the regression is real. Restored from `document_summary_backup_20260827` and
re-measured: every lane 0.000 — the revert is exact, not approximate.

### Why it lost, which is the part worth keeping

A YouTube title is marketing copy. "before it's too late", "Nobody is Talking About",
"(RAW RESULTS)" — prefixing pushes that into the vector that represents the document,
and it displaces genuine topical signal in a fixed-dimension embedding. Dense lost most
because it is purely the summary vector; reranked inherited the damage through it.

**The improvement it was meant to buy did not show up either**, and that is the more
interesting half. Lexical moved +0.005 — noise. The eval's 12 queries are topical
("how has the argument for local models developed"), not the find-the-known-thing query
that motivated the change. So this run establishes that titles hurt the *measured*
workload; it does not establish that titles never help the unmeasured one. The gap is
in the query set, and the honest fix is to add exact-recall queries to
`corpus/eval/queries.yaml` before re-litigating this — not to ship a change that fails
the gate on the strength of an anecdote.

**Rejected alternative — embed title+summary but store the summary alone.** Would have
spared the reranker and lexical the marketing copy. Rejected because it makes `text` and
`embedding` describe different things, which is exactly what the redo path's own comment
warns against: the dense lane would search one representation while the reranker reads
another, and nothing would fail when they disagreed.

**Still open.** Long-form conversational documents remain under-served by extractive
summarisation, and that is now a known, named weakness rather than a suspicion.
`SummaryMethod.ABSTRACTIVE_LLM` exists and is marked on-demand-only; a scoped abstractive
pass over just the long-form tier is the next candidate, and it needs its own gate run.
